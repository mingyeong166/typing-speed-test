...
function randomParagraph(){
    //getting random number and it'll always less than eht paragraphs lennth
    let randIndex = Math.floor(Math.random() * paragraphs.length);
    typingText.innerHTML = "";   
    //getitng randonm item from the paragraphs array, splitting all characters
    //of it, adding each character inside span and then adding this 
...
function initTyping(){
    ...
    if(charIndex < characters.length - 1 && timeLeft > 0){ 
        ...
        //if user hasn't entered any charater or pressed backspace
        if(typedChar == null){
            ...
            charIndex++; //incorrect charIndex either user typed correct or incorrect charater 
        }
...
function resetGame(){
    //calling loadParagraph functio  and
    //resetting each variavles and elements value to dafualt
    randomParagraph();
    inpField.value = "";
    clearInterval(timer);
    timeLeft = maxTime,
